...
async function loadPcrsFromApi(userId) {
  if (!userId) return;
  setPcrsLoading(true);
  try {
...
useEffect(() => {
  ...
  loadPcrsFromApi(loggedInUser.id);
}, [loggedInUser?.id]);
...
const goToPage = (pageNumber) => {
  ...
  // Use backend country codes (IN, JP, etc.) so API accepts; brands come from Product 360 / MDGM
  const countryBrandMap = {
    IN: ['EUTHYROX', 'Crocin', 'Saridon', 'Vicks'],
    JP: ['EUTHYROX', 'Dart', 'Crocin'],
    AL: ['EUTHYROX', 'Dart'],
    BA: ['EUTHYROX', 'Crocin'],
  };
  const countries = [
  ...
  const skus = [
    ...
    { value: 'SKU-IN-001', label: 'SKU-IN-001' },
  ];
  const countryLabel = (code) => ({ IN: 'India', JP: 'Japan', AL: 'Albania', BA: 'Bosnia' })[code] || code;
  // Brand -> therapeutic area (for current/floor price API when creating PCR)
  const brandToTherapeuticArea = { EUTHYROX: 'CMC', Dart: 'Cardiology', Crocin: 'OTC', BAVENCIO: 'Fertility & NDD', Saridon: 'OTC', Vicks: 'OTC' };
...
function handleSkuChange(skuValue) {
  ...
  // When creating PCR: fetch current + floor price for each selected SKU (show in modal)
  useEffect(() => {
    const country = newEntry.country;
    const brand = newEntry.brand;
    const skuIds = newEntry.sku;
    const ta = brand ? (brandToTherapeuticArea[brand] || 'CMC') : '';
  ...
  }, [newEntry.country, newEntry.brand, newEntry.sku.join(',')]);

 function handleAddEntry() {
  if (!newEntry.country || !newEntry.brand || newEntry.sku.length === 0) {
    alert('Please fill all fields and select at least one SKU');
    return;
  }
  ...
  const updatedData = [newPCR, ...allTableData];
  setAllTableData(updatedData);
  
  setSelectedRow(newPCR);
  setShowDetailPage(true);

  setShowNewModal(false);
  setNewEntry({ country: '', brand: '', sku: [] });
}
  ...
  function handleEscalateToGlobal(pcrId) {
    const updatedData = allTableData.map(row => {
      if (row.id === pcrId && row.approvalStatus === 'Pending' && !row.escalatedToGlobal) {
        return {
          ...row,
          approvalStatus: 'Escalated to Global',
          escalatedBy: loggedInUser?.email,
          escalatedDate: getCurrentDate(),
          escalatedToGlobal: true
        };
      }
      return row;
    ...
    alert('PCR escalated to global level for final decision! Regional approve/reject buttons are now disabled.');
  }
  ...
  async function handleResubmit(pcrId) {
    const pcr = allTableData.find(row => row.id === pcrId);
    if (!pcr) return;
    const canResubmit = pcr.approvalStatus === 'Rejected by Local' || 
                        pcr.approvalStatus === 'Rejected by Regional' || 
                        pcr.approvalStatus === 'Rejected by Global' || 
                        pcr.approvalStatus === 'Submitted for Local Approval' || 
                        pcr.approvalStatus === 'Pending';
          ...
          <div className="bg-white rounded-lg shadow-xl p-6 w-96 max-h-[90vh] overflow-y-auto">
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                >
                  <option value="">Select country (use IN for India to match backend)</option>
                  {countries.map((c) => (
  ...
  {newEntry.country &&
    countryBrandMap[newEntry.country]?.map(brand => (
    ...
    {skus.map(s => (
      <div key={s.value} className="flex items-center">
        <input
          ...
          id={s.value}
          value={s.value}
          checked={newEntry.sku.includes(s.value)}
          onChange={() => handleSkuChange(s.value)}
          disabled={!newEntry.brand}
          className="mr-2 h-4 w-4 text-purple-600 focus:ring-purple-500 border-gray-300 rounded"
        />
        <label 
          htmlFor={s.value} 
          className={`text-sm ${!newEntry.brand ? 'text-gray-400' : 'text-gray-700 cursor-pointer'}`}
        >
          {s.label}
        </label>
      </div>
  ...
  {newEntry.sku.length > 0 && (
    <p className="mt-2 text-sm text-purple-600">
      Selected: {newEntry.sku.join(', ')}
              ...
              {/* Current & floor price (from backend) when country + brand + SKU selected */}
              {newEntry.country && newEntry.brand && newEntry.sku.length > 0 && (
          ...
          <div className="bg-white rounded-lg shadow overflow-hidden">
            ...
            <div className="bg-gray-50 p-4 border-b flex items-center gap-2">
             ...
             <div className="ml-auto flex gap-2">  
              ...
              <button
                onClick={() => loggedInUser?.id && loadPcrsFromApi(loggedInUser.id)}
