    ...
    fetchAuditTrail(loggedInUser.id, { brand: selectedBrand || undefined, country: country || undefined })
      ...
      .catch((e) => {
        if ((e as Error).message === "FORBIDDEN") setAuditForbidden(true);
          ...
          setAuditEntries([]);
          setAuditError("Failed to load audit trail. (1) Start the backend from the Backend folder: python app.py — it must run on port 5000. (2) Only Admin users can view the Audit Trail; log in as Admin.");
        }
      })
      ...
      <main className="flex-1 overflow-auto p-6">
        ...
        {activeTab === "audit" && (
          <div className="rounded-lg border border-gray-200 bg-white shadow-sm">
            ...
            {auditForbidden && <p className="p-6 text-sm text-amber-700">Only Admin can view the Audit Trail. You do not have access.</p>}
